...
public class StatementWorker {
    ...
    public static ArrayList sortBankArrayList(ArrayList transactionDataList, boolean reverse) throws Exception {
        ...
        try {
            ...
            LOG.trace("[WebSync:StatementWorker] verändertes Original des reversed-transactionDataList enthält nun " +
                    "das MatrixArray:\n" + StringUtils.formatedArrayListForLog(transactionDataList));
            LOG.trace("[WebSync:StatementWorker] sollte gleiche copyUnsortedDataList sein und enthält nun das " +
                    "MatrixArray:\n" + StringUtils.formatedArrayListForLog(copyUnsortedDataList));
            Collections.sort(copyUnsortedDataList, new StringUtils.MatrixArrayListDateComparator());
            LOG.trace("[WebSync:StatementWorker] sollte das nach Datum sortierte copyUnsortedDataList sein und " +
                    "enthält nun das MatrixArray:\n" + StringUtils.formatedArrayListForLog(copyUnsortedDataList));
            ...
            LOG.trace("[WebSync:StatementWorker] Liste aller abgeholten Umsätze: (schon fertig formatiert und " +
                    "sortiert, bereit zur Verarbeitung):\n" + StringUtils.formatedArrayListForLog(transactionDataList));
            return transactionDataList;
        } catch (Exception var14) {
            StringWriter sw = new StringWriter();
            var14.printStackTrace(new PrintWriter(sw));
            LOG.error("[WebSync:StatementWorker] Fehler beim Verarbeiten transactionDataList und " +
                    "Umkehr-Copy-Sortier-Funktion\n" + sw.toString());
            throw new Exception("Fehler beim Verarbeiten transactionDataList und Umkehr-Copy-Sortier-Funktion; " + var14.getMessage());
        }
    }

    public static ArrayList createBankArrayListHeader(String InstitutAlias, String AccountNr, double Amount,
                                                      double avalibleAmount, double unbookedAmount,
        ...
        ((ArrayList) BankArrayListHeader.get(0)).add("Automatisch generierter Kontoauszug der " +
                "Java-ScreenScrapingBibliothek 'finance.websync' (beinhaltet die sortierterten, fertig formatierten " +
                "Umsätze)");
        BankArrayListHeader.add(new ArrayList());
        BankArrayListHeader.add(new ArrayList());
