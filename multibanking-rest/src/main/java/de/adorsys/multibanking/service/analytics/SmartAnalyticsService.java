...
public class SmartAnalyticsService {
    ...
    public AnalyticsResult analyzeBookings(String userId, List<BookingEntity> bookings, LocalDate referenceDate) {
        bookings.forEach(bookingEntity -> bookingEntity.setBookingCategory(null));
        List<Rule> customRules = loadUserRules(userId);

        AnalyticsRequest analyticsRequest = createAnalyticsRequest(bookings, referenceDate, customRules);
    ...
    private AnalyticsRequest createAnalyticsRequest(List<BookingEntity> bookings, LocalDate referenceDate, List<Rule> customRules) {
